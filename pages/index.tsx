  ...
  const filteredBlog: BlogPost[] = useMemo(() => {
    return filterWord.length > 0
      ? blogData.filter((blog: BlogPost) => {
          return filterWord.every((filter) => {
    ...
    <main className="layout">
      ...
      <section>
        <div className="mt-3 text-center">
          <h1 className="text-[3rem]">Welcome to ThachDevBlog</h1>
          <p>
      ...
      <section className="flex flex-col items-center text-[1.15rem] mt-12">
        ...
        {filteredBlog.map((blog: BlogPost) => {
          return (
            <div
              key={blog.id}
              className="max-w-[28em] max-h-[20em] overflow-hidden mx-6 mb-6 bg-neutral-200 text-zinc-800 rounded-lg p-4 hover:bg-neutral-500 transition-all duration-300"
            >
